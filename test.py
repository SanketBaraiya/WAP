...
def check_files_name(directory, target_file):
    for root, dirs, files in os.walk(directory):
...
def amcache(path, zip=False):
    ...
    amcache_path = check_files_name(base_directory, "Amcache.hve")
...
def browser_data(path, zip=False):
    ...
    edge_paths = []
    firefox_paths = []
    results = find_files(base_directory, "History")
    if results != None:
        for path in results:
    ...
    results = find_files(base_directory, "places.sqlite")
    if results != None:
        for path in results:
...
def jumplist(path, zip=False):
    ...
    jumplist_custom = check_files_extension(base_directory, ".customDestinations-ms")
    jumplist_automatic = check_files_extension(base_directory, ".automaticDestinations-ms")
...
def mft(path, zip=False):
    ...
    mft_path = check_files_name(base_directory, "$MFT")
...
def usnlog(path, zip=False):
    ...
    usn_path = check_files_name(base_directory, "$J")
    log_path = check_files_name(base_directory, "$LogFile")
...
def prefetch(path, zip=False):
    ...
    prefetch_present = check_files_extension(base_directory, ".pf")
...
def recent_files(path, zip=False):
    ...
    recent_files_present = check_files_extension(base_directory, ".lnk")
...
def shellbags(path, zip=False):
    ...
    ntuser_present = check_files_name(base_directory, "NTUSER.DAT")
    usrdat_present = check_files_name(base_directory, "UsrClass.dat")
...
def registry(path, zip=False):
    ...
    software_hive_present = check_files_name(base_directory, "SOFTWARE")
    system_hive_present = check_files_name(base_directory, "SYSTEM")
    sam_hive_present = check_files_name(base_directory, "SAM")
    security_hive_present = check_files_name(base_directory, "SECURITY")
...
def event_logs(path, zip=False):
    ...
    event_logs_present = check_files_extension(base_directory, ".evtx")
